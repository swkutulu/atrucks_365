...
сделал через COPY.
Посмотрел профайлером скртипт с pandas 350Mb (и от файлов зависит), c COPY 131 Mb.
...
Выбор пандас изначально обусловлен реальной задачей и данными и затраченным временем, а не сферическими конями в безвоздушном пространстве.
Нормализация с pandas заняла минут 30-40, с sql 2 часа: подзабыл, а кое что и не использовал никогда.
Ну и валидация с пандас проще.
...
Немного оне так как хотел, сделал докер, этот только для демонстрации, хотелось полноценно инициализацию базы, создание пользователя и первоначальную загрузку сделать.
